...
class EventsController extends FullCalendarAppController {
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Event->save($this->data)) {
				...
				$this->redirect("/full_calendar/");
			} else {
				$this->Session->setFlash(__('The event could not be saved. Please, try again.', true));
			}
		}
		$this->set('eventTypes', $this->Event->EventType->find('list'));
		$this->set('users', $this->Event->ExecutedBy->find('list'));
	...
	function feed($id=null) {
		...
		$conditions = array('conditions' => array('executed_by'=>$this->Session->read('Auth.User.id'),	'UNIX_TIMESTAMP(start) >=' => $vars['start'], 'UNIX_TIMESTAMP(start) <=' => $vars['end']));
